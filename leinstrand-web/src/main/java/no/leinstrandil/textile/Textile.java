...
public class Textile implements TextileConstants {
    ...
    public final String process(final String origionalContent) {
        ...
        /**
         * trim each line.. no it is not faster to String.split() here since we
         * are just trimming fat
         */
        ...
        for (int x = 0; x < EXP_PHRASE_MODIFIER_SOURCETAGS.length; x++) {
            String prefix = "(^|\\s|>)" + EXP_PHRASE_MODIFIER_SOURCETAGS[x] + "(.+?)([^\\w\\s]*?)"
                    + EXP_PHRASE_MODIFIER_SOURCETAGS[x] + "([^\\w\\s]{0,2})?"; // (\\s|$)?";

            // String suffix = "$1<" + EXP_PHRASE_MODIFIER_REPLACETAGS[x] +
            // ">$2$3</" + EXP_PHRASE_MODIFIER_REPLACETAGS[x] + ">$4";
            ...
            content = matcher.replaceAll(suffix);

            content2 = content2.replaceAll(prefix, suffix);
        }

        /**
         * From the Origional Docs: "some weird bs with underscores and \b word
        ...
        String[] glyphMatches = { EXP_SINGLE_CLOSING, EXP_SINGLE_OPENING, EXP_DOUBLE_CLOSING, EXP_DOUBLE_OPENING,
                EXP_ELLIPSES, EXP_3UPPER_ACCRONYM, EXP_3UPPERCASE_CAPS, EXP_EM_DASH, EXP_EN_DASH, EXP_EN_DECIMAL_DASH,
                EXP_DIMENSION_SIGN, EXP_TRADEMARK, EXP_REGISTERED, EXP_COPYRIGHT };

        String[] glyphReplacement = { REPLACE_SINGLE_CLOSING, REPLACE_SINGLE_OPENING, REPLACE_DOUBLE_CLOSING,
                REPLACE_DOUBLE_OPENING, REPLACE_ELLIPSES, REPLACE_3UPPER_ACCRONYM, REPLACE_3UPPERCASE_CAPS,
                REPLACE_EM_DASH, REPLACE_EN_DASH, REPLACE_EN_DECIMAL_DASH, REPLACE_DIMENSION_SIGN, REPLACE_TRADEMARK,
                REPLACE_REGISTERED, REPLACE_COPYRIGHT };
        ...
        String[] blockMatches = { EXP_BULLETED_LIST, EXP_NUMERIC_LIST, EXP_BLOCKQUOTE, EXP_HEADER_WITHCLASS,
                EXP_HEADER, EXP_PARA_WITHCLASS, EXP_PARA, EXP_REMAINING_PARA };

        String[] blockReplace = { REPLACE_BULLETED_LIST, REPLACE_NUMERIC_LIST, REPLACE_BLOCKQUOTE,
                REPLACE_HEADER_WITHCLASS, REPLACE_HEADER, REPLACE_PARA_WITHCLASS, REPLACE_PARA, REPLACE_REMAINING_PARA };
